...
- 생성용 스크립트는 기본적으로 새로 만들지 않습니다. 테스트 fixture/mock 데이터 생성을 위한 스크립트나 helper는 테스트 코드로 취급해 해당 서비스의 `tests/` 또는 테스트 패키지 안에만 둡니다.
- 생성용 스크립트가 운영 로직에 반드시 필요한 경우에만 예외적으로 유지할 수 있습니다. 이때 생성 산출물의 소유 서비스 아래에 두고, 왜 런타임/수동 상수/테스트 fixture로 대체할 수 없는지 관련 README 또는 모듈 문서에 명시합니다.
...
- 생성용 스크립트는 기본 금지입니다. 추가하기 전에 생성 대상이 테스트 fixture/mock인지, 운영 로직 필수 산출물인지 먼저 분류합니다.
- 테스트 fixture/mock 생성기는 테스트 자산입니다. 서비스 루트나 운영 모듈에 두지 말고 해당 서비스의 `tests/` 또는 테스트 패키지 안에 둡니다.
- 운영 로직에 빠지면 안 되는 필수 생성기만 생성 산출물을 소유한 서비스에 둘 수 있습니다. 해당 생성기는 산출물, 입력 소스, 실행 시점, 수동 편집 금지 여부, 예외로 유지하는 이유를 문서화해야 합니다.
- 네트워크나 외부 소스에 의존하는 생성기는 자동 검증 경로에 넣지 않고, 사용자가 명시적으로 요청했을 때만 실행합니다.
