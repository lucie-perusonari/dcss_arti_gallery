...
from api.admin_repository import create_mongo_crawl_status_repository
from api.repository import create_mongo_read_repository
...
def mongo_repository_for_test(prefix: str):
    ...
    repository = create_mongo_read_repository(
        uri=env["MONGODB_URI"],
...
def insert_artifacts(repository, artifacts: list[dict]) -> None:
    if artifacts:
        repository.collection.insert_many(artifacts)
...
def _wait_for_mongo(repository) -> None:
    _wait_for_collection(repository.collection)
