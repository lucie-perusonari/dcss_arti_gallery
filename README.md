...
- [crawl_service](crawl_service/README.md): remote morgue discovery/fetch, raw file 저장, 저장된 원본 처리, artifact document 생성, MongoDB write, background worker
- [api](api/README.md): MongoDB artifact/admin read API, API-owned Pydantic response DTO, FastAPI routes
- [frontend](frontend/README.md): React/Vite WebTiles-style artifact gallery
- [admin-frontend](admin-frontend/README.md): React/Vite crawl operations dashboard
...
모듈별 세부 문서는 각 모듈의 `docs/`에 둡니다. 루트 [docs](docs/README.md)는 모듈 docs와
운영/harness 문서로 가는 인덱스 역할을 합니다.

주요 공유 문서:

- [Reference Index](docs/reference/README.md): 모듈별 reference 문서 인덱스
- [Crawl Service Data Types](crawl_service/docs/reference/data-types.md): crawl/parser/scoring/storage 타입
- [API Data Types](api/docs/reference/data-types.md): Gallery/admin API response DTO
- [Frontend Data Types](frontend/docs/reference/data-types.md): gallery TypeScript artifact 타입
- [Artifact Scoring Formula](crawl_service/docs/reference/artifact_scoring_formula.md): artifact 점수 산정 기준
- [DCSS Item Data](crawl_service/docs/reference/dcss_item_data.md): 공식 DCSS item data refresh 기준
- [Harness Validation](docs/ops/harness/validation.md): scoped/cross-project validation matrix
