...
const weaponCategoryOrder = ['short blades', 'long blades', 'axes', 'maces & flails', 'polearms', 'staves', 'ranged', 'other weapons'];
...
const luxuryShieldBases = new Set(['buckler', 'kite shield', 'shield', 'tower shield']);
...
export function App() {
  ...
  const displayedArtifacts = useMemo(() => artifactsMatchingFilters(artifacts, filters), [artifacts, filters]);
...
function slotOptionsForArtifacts(artifacts: Artifact[], type: ArtifactFilters['type'], luxuryOnly: boolean) {
  ...
  const sortedSlots =
    type === 'weapon'
      ? Array.from(slots).sort((left, right) => weaponCategoryRank(left) - weaponCategoryRank(right) || left.localeCompare(right))
...
function artifactsMatchingFilters(artifacts: Artifact[], filters: ArtifactFilters) {
...
function jewellerySlotForArtifact(artifact: Artifact) {
  ...
  if (jewellerySlot === 'amulet' || baseItem === 'amulet' || subtype.startsWith('amulet of ')) return 'amulet';
  return 'ring';
}
