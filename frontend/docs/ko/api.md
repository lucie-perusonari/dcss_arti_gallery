...
`listArtifacts`는 type/search/player/timeRange 기준 원본 목록을 반환합니다. `timeRange`는 Gallery API의
`since` query parameter로 전달하며, 기본값은 최근 30일입니다. armour slot, weapon subtype,
