...
- Gallery frontend, Gallery API, Admin API, Admin frontend, MongoDB, Prometheus, Grafana, reverse proxy를 묶는 Docker Compose 배포 예시를 제공합니다.
...
- `admin-frontend`: Caddy로 Admin frontend 정적 파일과 `/admin-api/*` Admin API proxy를 host loopback에만 제공합니다.
...
Grafana, Admin API, Admin frontend port는 공유기에서 직접 열지 않습니다.
...
운영 배포 전에는 반드시 `develop`에서 같은 commit을 개발 서버에 먼저 배포하고 확인합니다.
확인한 commit만 `main`으로 fast-forward하고, 작업트리가 비어 있는 상태에서 annotated tag와 GitHub Release를
만든 뒤 운영 compose를 재빌드합니다.
...
Admin frontend UI는 서버 내부 `127.0.0.1:5174`에만 노출합니다.
...
admin frontend에 Grafana 링크를 표시하려면 prod compose build 환경에 `VITE_GRAFANA_URL`을 지정합니다.

```sh
VITE_GRAFANA_URL=http://127.0.0.1:3000/d/dcss-gallery-api/dcss-gallery-api docker compose -f infra/prod/docker-compose.yml up -d --build admin-frontend
```
