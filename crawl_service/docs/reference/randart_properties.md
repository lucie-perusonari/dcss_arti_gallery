...
| --- | --- | --- | --- |
| `Slay` | slaying modifier | signed integer | 무기와 스태프에는 랜덤 추가 속성으로 붙지 않음 |
| `MP` | max MP modifier | signed integer | 무기와 스태프에는 랜덤 추가 속성으로 붙지 않음 |
...
| `rElec` | electricity resistance | boolean | 없음 |
| `rPois` | poison resistance | boolean | 없음 |
| `rN` | negative energy resistance | positive integer | 없음 |
| `Will` | willpower | signed integer | willpower penalty |
| `rCorr` | corrosion resistance | boolean | 없음 |
...
## Displayed Intrinsic Properties
...
## Randomly Generated Property Set
...
## Properties Not In The General Random Pool
