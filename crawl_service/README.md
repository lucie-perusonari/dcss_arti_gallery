...
crawl file/user cache 기록, one-shot worker를 소유합니다.

HTTP API를 제공하지 않는 one-shot worker 프로젝트이며, `api`, `frontend`, `admin-frontend`, `arti_parser`가 직접 의존해야 하는 대상이 아닙니다.

## 모듈

- [`fetcher.py`](docs/ko/fetcher.md): remote morgue root/user directory 조회, txt/lst 파일 목록 추출, 파일 본문 fetch
- [`repository.py`](docs/ko/repository.md): `raw_morgue_files`, `crawl_errors`에 morgue ingest 상태 저장
...
- one-shot worker로 archive user list scan과 raw morgue file ingest를 조율합니다.
...
3. 이미 저장된 fetched raw file은 skip합니다.
4. 새 파일 원문을 `raw_morgue_files`에 저장합니다.
5. fetch 실패 이벤트는 `crawl_errors`에 append-only 로그로 기록합니다.
artifact parsing, scoring, artifact document 생성, `artifacts` 저장은 `arti_parser` 책임입니다.
...
worker는 archive의 전체 user directory list를 한 번 훑고, 대상 user directory를 열어 누락 raw file을 확인한 뒤 종료합니다.
2026-01-01 이후 파일 데이터만 처리하며 모든 HTTP 요청 사이에 기본 1초 delay를 둡니다.
