...
PID_FILE="${PID_FILE:-$LOG_DIR/crawl_raw_only_dev.pid}"
LOG_FILE="${LOG_FILE:-$LOG_DIR/crawl_raw_only_dev.log}"
DETACH="${DETACH:-0}"
...
if [ "$DETACH" = "1" ]; then
  ...
  if [ -f "$PID_FILE" ]; then
    ...
    if [ -n "$existing_pid" ] && kill -0 "$existing_pid" 2>/dev/null; then
      echo "Dev raw morgue crawler is already running."
  ...
  worker_pid=$!
  printf '%s\n' "$worker_pid" > "$PID_FILE"
  echo "Dev raw morgue crawler started in background."
  echo "  PID:  $worker_pid"
  echo "  Log:  $LOG_FILE"
...
echo "Starting dev raw morgue crawler."
